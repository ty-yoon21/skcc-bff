...
# SKEP STD - Wijmo Custom Function
        ...
        cellToDateRangePicker           - skepStdWijmo.flexGrid.cellToDateRangePicker(fg, column)
...
skepStdWijmo.flexGrid = {
    ...
    init: function (id, options, rownum = true, statusbar = true) {

        const _options = {
            showSort: true,
            allowSorting: 2,  // 0:None,1:SingleColumn,2"MultiColumn
            deferResizing: true,
            autoGenerateColumns: false,
            imeEnabled: true, // 한영 변환 버그X
            showMarquee: false,
        ...
        var fg = new wijmo.grid.FlexGrid(id, _options);
        fg.itemsSource.refreshOnEdit = false;    // 셀 편집후 정렬 x
        const rowHeight = 24;

        // 순번 적용
        // rowheaders[0] = rownum
        // rowheaders[1] = Selector
        fg.rownum = rownum;
        fg.rowHeaders.columns[0].visible = false;
        if(rownum) {
            fg.rowHeaders.columns[0].binding = '_rownum';
            fg.rowHeaders.columns[0].width = 45;
            fg.rowHeaders.columns[0].align = 'center';
            fg.rowHeaders.columns[0].visible = true;
            fg.rowHeaders.columns[0].allowMerging = true;
            fg.rowHeaders.columns[0].header = '순번';
            fg.rowHeaders.columns[0].cssClassAll = '_rownum';

            // merge가 안돼서 css 수정
            fg.updatedView.addHandler(function() {
                if(fg.columnHeaders.rows.length > 1) {
                    const topLeft = $(id + ' .wj-topleft ._rownum');
                    $(topLeft[0]).css('height', rowHeight * fg.columnHeaders.rows.length);
                    $(topLeft[0]).before($(topLeft[0]).text("순번"));
                    for(let i = 1; i < topLeft.length; i ++) {
            ...
            fg._cv.sourceCollectionChanged.addHandler(function (cv, e) {
                cv.items.map((item, i) => item._rownum = i + 1);
            });
            ...
            // sorting 사용할 때
            fg.sortedColumn.addHandler(function(g, e) {
                g.collectionView.items.map((c, i) => c._rownum =  i + 1);
            });
        }
        ...
        fg.columnHeaders.rows.defaultSize = rowHeight;
        ...
        // number에 maxLength 적용하기 위해 수정
        fg.addEventListener(fg.hostElement, 'keydown', function() {
            const input = fg.activeEditor;
            if(!input) return;

            if(input.type == 'number' && !input.oninput) {
        ...
        //     });
        // }
        fg.selectionChanged.addHandler(function (fg, e) {
            // tab으로 이동시 date format 적용 위해 추가
            if(e.col > 1 && $(fg.cells.getCellElement(e.row, e.col - 1)).data('daterangepicker')) {
                fg.refresh();
            }
        });
...
//        });
    ...
    ,updateStatusbar : function(fg) {
        if (fg.statusbar) {
            if (fg.allowAddNew) {
                fg.statusbar.innerText = "Result : "+wijmo.Globalize.format(fg.itemsSource.items.length-1 ,'n');
            } else {
                fg.statusbar.innerText = "Result : "+wijmo.Globalize.format(fg.itemsSource.items.length ,'n');
            }
        }
    }
     ...
     */
    ,rowChecker: function (fg,hiddenCheckAll) {

        fg.headersVisibility = 3;
        fg.rowHeaders.columns.push(new wijmo.grid.Column());
        
        fg.selector = new wijmo.grid.selector.Selector(fg.rowHeaders.columns[1]);
        if (hiddenCheckAll) fg.selector.showCheckAll = false;
        fg.selector.column = fg.rowHeaders.columns[1];
        fg.rowHeaders.columns[1].allowMerging = true;
        fg.rowHeaders.columns[1].align = 'center';
    }
    /**
...
//        fg._cv.commitNew();
        ...
        //var index = (fg.rows.length>0 && fg.rows[0].cssClass === 'summaryRow') ? 1 : 0;
        var index = 0;
        var newItem = {};
        fg._cv.sourceCollection.splice(index, 0, newItem);
        fg._cv.refresh();

        fg.select(new wijmo.grid.CellRange(index, 0), true);
    }
    /**
...
//        var summaryRowIndex = fg.rows.findIndex(row => row.cssClass && row.cssClass.includes('summaryRow'));
//        var insertIndex = summaryRowIndex >= 0 ? summaryRowIndex + 1 : 0;
//        fg._cv.sourceCollection.splice(insertIndex, 0, param);
//        fg._cv.refresh();
//        fg._cv.moveCurrentToPosition(0);
    }
    /**
     * delRow - skepStdWijmo.flexGrid.delRow(fg1);
     */
    ,delRow: function (row, grid) {

        var fg = grid ? grid : row.grid;
    ...
    ,delRows: function (fg) {
        ...
        var grid = (dRows.length > 0) ? dRows[0].grid : null;
        
        dRows.forEach(function (row) {
           skepStdWijmo.flexGrid.delRow(row, grid);
       });
    }
    /**
     * isAddedRow - skepStdWijmo.flexGrid.isAddedRow(e.getRow())
    ...
    ,cellToDateRangePicker: function(fg, column, type) {
        /**
         * fg: wijmo.grid.FlexGrid
         * column: wijmo.grid.Column
         * type: default - 기본 Single Calendar,
         *       month - 월별 Month Calendar,
         */
        ...
        column.cellTemplate = (ctx) => `${formatDateToView(ctx.text)}<i class="mdi mdi-calendar" style="float: right;"></i>`;

        column.dateColumn = true;  // date Column으로 설정
        column.inputType = 'number';
        column.maxLength = 8;

        // fg.cells.getCellElement(ctx.row.index, ctx.col.index)?.classList.contains('fgEditableCell') ? `${ctx.text}<i class="mdi mdi-calendar" style="float: right;"></i>` : ctx.text
        if(type == 'month' || type == 'monthRange') {
            column.cellTemplate = (ctx) => `${formatDateToView(ctx.text)}<i class="mdi mdi-calendar-month" style="float: right;"></i>`;
        }

        fg.addEventListener(fg.hostElement, 'click', function(e) {

            const h = fg.hitTest(e);
            if(h.row == -1 || h.col == -1) {return;}
            const cell = fg.cells.getCellElement(h.row, h.col);
            let dp = $(cell).data('daterangepicker');
            const _column = fg.getColumn(h.col);

            // 열헤더의 체크박스 선택시에도 Return 처리 h.cellType==3
            if ( (h.cellType==3) || (_column.dateColumn=='undefined') || (_column.dateColumn==null) || (_column.dateColumn!=null && !_column.dateColumn) ) {
                if(dp) dp.hide();
                return;
            } else {
                if (cell.ariaReadOnly!=null && cell.ariaReadOnly=="true") {
                    if(dp) dp.hide();
                    return;
                }
            }
            ...
            if(!dp) {
                ...
                $(cell).data('fgRow', h.row);
                $(cell).data('fgCol', h.col);

                if(type == 'month') {
                    const regex1 = /^\d{4}\d{2}$/; //yyyymm 형식
                    const regex2 = /([12]\d{3}(0[1-9]|1[0-2]))/

                    column.maxLength = 6;
                    $(cell).daterangepicker(daterangeOptions);
                    dp = $(cell).data('daterangepicker');

                    $(cell).on('apply.daterangepicker', function(ev, picker) {
                        fg.collectionView.editItem(fg.rows[h.row].dataItem);
                        picker.setStartDate(picker.leftCalendar.month);
                        fg.rows[h.row].dataItem[_column.binding] = picker.leftCalendar.month.format('YYYYMM');
                        fg.onCellEditEnded(new wijmo.grid.CellRangeEventArgs(fg.cells, fg.selection, picker.leftCalendar.month.format('YYYYMM')));
                        fg.collectionView.commitEdit();
                        fg.refresh();
                    });
                    $(cell).on('showCalendar.daterangepicker', function(ev, picker) {
                        $('.table-condensed tbody, .table-condensed thead tr:last-child').hide();
                        fg.refresh();
                    });

                    fg.cellEditEnding.addHandler(function(s, e) {
                        const col = s.columns[e.col];
    
                        if(s.activeEditor != null && col.binding == column.binding) {
                            if(s.activeEditor.value && (!regex1.test(s.activeEditor.value) || !regex2.test(s.activeEditor.value))) {
                                e.cancel = true;
                            }
                        }
                    });
                    ...
                    $(cell).on('apply.daterangepicker', function(ev, picker) {
                        fg.collectionView.editItem(fg.rows[h.row].dataItem);
                        fg.rows[h.row].dataItem[_column.binding] = picker.startDate.format('YYYYMMDD');
                        fg.onCellEditEnded(new wijmo.grid.CellRangeEventArgs(fg.cells, fg.selection, picker.startDate.format('YYYYMMDD')));
                        fg.collectionView.commitEdit();
                        fg.refresh();
                    });

                    $(cell).on('cancel.daterangepicker', function(ev, picker) {
                        fg.collectionView.editItem(fg.rows[h.row].dataItem);
                        fg.rows[h.row].dataItem[_column.binding] = '';
                        fg.onCellEditEnded(new wijmo.grid.CellRangeEventArgs(fg.cells, fg.selection, ''));
                        fg.collectionView.commitEdit();
                        fg.refresh();
                    });

                    const regex = /^\d{4}\d{2}\d{2}$/; //yyyymmdd 형식
                    ...
                    fg.cellEditEnding.addHandler(function(s, e) {
                        ...
                        // 날짜 유효성 검사
                        if(s.activeEditor != null && col.binding == column.binding) {
                            if(s.activeEditor.value && !regex.test(s.activeEditor.value)) {
                                e.cancel = true;
                            } else if(s.activeEditor.value && regex.test(s.activeEditor.value)) {
                                const value = s.activeEditor.value.toString();
                                const dtYear = value.slice(0, 4);
                                const dtMonth = value.slice(4, 6);
                                const dtDay = value.slice(6, 8);
                ...
                if(fg.cells.getCellElement(h.row, h.col)?.classList.contains('fgEditableCell')) {
                    dp.toggle();
                }
     ...
     */
    ,excelExport : function(fg, fileNm, header, columns = true) {
        // columns : boolean, Array
        // 0 : 전체컬럼 출력,
        // 1 : visible=true인 컬럼만 출력
        // columns를 지정하는 경우 해당 컬럼만 출력
           ...
           ,{ includeColumnHeaders: (header==undefined)?true:header,
                includeColumns: function(column) {
                    ...
                    if(columns && Array.isArray(columns)) {
                        if(columns.includes(column.binding)) {
                            return true;
                        } else {
                            return false;
                        }
    ...
    */
    ...
    /**
     * excelExportWithHiddenColumns
     * Ex)
     * - skepStdWijmo.flexGrid.excelExport(fg1, 'CommonCodeMgmt-CDCL');
     * - skepStdWijmo.flexGrid.excelExport(fg1, 'CommonCodeMgmt-CDCL', false);
     */
    ,excelExportWithHiddenColumns : function(fg, fileNm, header) {

        var fn = fileNm||'ExcelExport';
            fn = fn + "_"+ Date.now().toString() + ".xlsx"

        // 기존의 컬럼 visible 속성을 백업.
        var originalVisibility = [];
        fg.columns.forEach(function (col, index){
            originalVisibility[index] = col.visible;
            col.visible = true;
        });
        ...
        wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync(
           ...
           ,{ 
                ...
                //     //visble:false의 column data를 강제로 포함
                //     if(args.panel && args.panel.columns && args.panel.columns.length > args.col){
                //         if(!args.panel.colums[args.col].visible){
                //             args.vale = fg.getCellData(args.row, args.col, true);
                //         }
                //     }
                // }
            }
           ...
           //,null
           ,function() {
                //내보내기 완료 후 visible 속성 복원
                console.log('내보내기 완료!!');
                fg.columns.forEach(function(col, index){
                    col.visible = originalVisibility[index];
                });            
           }
        //    ,() => {
        //     //내보내기 완료 후 visible 속성 복원
