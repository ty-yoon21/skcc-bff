...
if (typeof (skep.ui) == "undefined") {
    ...
    skep.ui = {
        ...
        layerOpen: function(el) {
            ...
            //TO-DO --> 사업소관리 System은 Localize사용하지 않음
...
if (typeof (skep.ui.msg) == "undefined") {
    ...
    skep.ui.msg = {
        ...
        confirmWihReason: function(content, content_i18n_key, title, title_i18n_key) {

            return this.createMsgBoxWithInput(content, content_i18n_key, null, 'YESNO', title, title_i18n_key);

        },
        createMsgBox: function (content, content_i18n_key, icon_type, button_type, title, title_i18n_key, buttons) {
            skep.ui.layerRemove('alert_content');
        ...
        createMsgBoxWithInput: function (content, content_i18n_key, icon_type, button_type, title, title_i18n_key, buttons) {
            skep.ui.layerRemove('alert_content');
            return new Promise(function (resolve, reject) {
                ...
                el[9] = document.createElement("textarea");
                el[9].id = 'reason_input';                
                el[9].className = "input--reason";
                el[9].placeholder = "사유를 입력 바랍니다.";
                el[9].style.width = "95%";
                el[9].style.display = "block";
                el[9].style.margin = "10px auto";
                el[9].style.height = "100px";
                el[9].onclick = function() {
                    reasonInput.classList.add('placeholder');
                }

                el[10] = document.createElement("div");
                el[10].className = "button__group--center";

                el[11] = document.createElement("button");
                el[11].className = "btn--primary";
                el[11].onclick = function() {
                    let reasonInput = document.getElementById("reason_input")
                    let reason = reasonInput.value;
                    if(reason.trim() === ''){
                        //alert("사유를 입력 바랍니다.");
                        //skep.ui.msg.alert("사유를 입력 바랍니다.");
                        //reasonInput.classList.add('highlight-placeholder');
                        reasonInput.style.setProperty('outline', '2px solid blue');
                        reasonInput.focus();
                        setTimeout(()=> reasonInput.blur(), 100);
                        return;
                    }
                    //resolve(true); 
                    resolve(reason); 
                ...
                el[11].innerHTML = "예";
                el[11].dataset.i18n = "stdframework.core.button.yes";

                el[12] = document.createElement("button");
                el[12].className = "btn__close btn--outline--gray-500";                
                el[12].innerHTML = "아니오";
                el[12].onclick = function() {
                    resolve(false); 
                    skep.ui.layerRemove('alert_content');
                }
                el[12].dataset.i18n = "stdframework.core.button.no";
                ...
                el[6].appendChild(el[7]);
                el[7].appendChild(el[8]);
                el[7].appendChild(el[9]);
                el[7].appendChild(el[10]);
                el[10].appendChild(el[11]);
                el[10].appendChild(el[12]);
                window.document.body.appendChild(el[0]);
                    ...
                    menuList.forEach(function(menu) {
                        ...
                        if (menu.url) {
                            ...
                            a.addEventListener("click", function (event) {
                                ...
                                /** bom 용 추가 (하위 메뉴 클릭(ex 자금>C/F 계획))*/
            ...
            menuList.forEach((menu) => {
                ...
                if (menu.url) {
                    ...
                    a.addEventListener("click", function (event) {
                        ...
                        /** bom 용 추가 (하위 메뉴 없는 메뉴 클릭 (ex 예제메뉴))*/
                        // let isOpenTab = document.querySelector('#menuTabsArea > ul > li > button') == null ? false:true;
                        // if(isOpenTab) {
                        //     document.querySelector('#menuTabsArea > ul > li > button').click();
                        // }
...
if (typeof (skep.ui.menuTab) == "undefined") {
    ...
    skep.ui.menuTab = function(el) {
        ...
        this.selectTab = function (id) {
            ...
            t.findTab(id).ca.style.width = parseInt(window.innerWidth)+'px';
            t.findTab(id).ca.style.height = (parseInt(window.innerHeight)-50)+'px';  // 50 : header 높이
        ...
        this.closeAllTab = function () {
            ...
            t.tabs.forEach(el => t.tabsArea.removeChild(el.ta))
            t.tabs.forEach(el => t.contentsArea.removeChild(el.ca))
            t.tabs = [];
        ...
        this.setSize = function (el) {
            ...
            let contentHeight = parseInt(window.innerHeight - t.config.headerHeight);
            ...
            for(let node of el.childNodes) {
                if(node.nodeName === 'IFRAME') {
                    ...
                    /** bom 용 수정.
                     * 페이지 로드 완료 후 이 함수 타서 페이지 높이를 조절하게 되어 여기서 셋팅 해야 하는듯함.
                     * 해당 페이지 스크립트에서 셋팅 할 수 없어서 이 부분 수정.
                     * 탭 존재 여부에 따라 height 다르게 설정
                     */

                    if(node.contentDocument.getElementById('tabPanel')) {  // 내부 탭 div
                        let tempHeight = node.contentDocument.getElementById('tabPanel').offsetHeight;
                        contentHeight = (contentHeight-tempHeight);
                    }

                    if(node.contentDocument.getElementById('commonSearch')) {  // 내부 탭 위 검색 div
                        let tempHeight = node.contentDocument.getElementById('commonSearch').offsetHeight;
                        contentHeight = (contentHeight-tempHeight);
                    }

                    node.contentDocument.getElementById("container").style.height = contentHeight +"px";
                    node.contentDocument.getElementById("container").style.width = el.style.width;
                }
            }
        }
